defmodule BlogifyWeb.OwnerForgotPasswordLive do
  ...
  def handle_event("send_email", %{"owner" => %{"email" => email}}, socket) do
    if owner = Accounts.get_owner_by_email(email) do
      Accounts.deliver_owner_reset_password_instructions(
        owner,
        &url(~p"/owners/reset_password/#{&1}")
      )
    end
